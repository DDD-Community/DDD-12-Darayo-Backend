...
public class AdminReservationSequentialE2ETest extends ContainerBaseTest {
    ...
    @Test
    void add_update_delete_reservation_updates_updatedAt() throws Exception {
        Long performanceId = 1L; // seeded

        // add reservation info
        LocalDateTime addStart = LocalDateTime.now(ZoneId.of("Asia/Seoul"));
        var addBody = Map.of(
                "openDateTime", LocalDateTime.now(ZoneId.of("Asia/Seoul")).plusDays(1).withNano(0).toString(),
                "closeDateTime", LocalDateTime.now(ZoneId.of("Asia/Seoul")).plusDays(2).withNano(0).toString(),
                "type", ReservationType.GENERAL.name(),
                "ticketURL", "https://ticket.example.com/new",
                "remark", "신규 예매"
        );
        mockMvc.perform(post("/api/admin/performance/{performanceId}/reservation", performanceId)
        ...
        // update seeded reservation id 1
        LocalDateTime updateStart = LocalDateTime.now(ZoneId.of("Asia/Seoul"));
        var updateBody = Map.of(
                "openDateTime", LocalDateTime.now(ZoneId.of("Asia/Seoul")).plusDays(3).withNano(0).toString(),
                "closeDateTime", LocalDateTime.now(ZoneId.of("Asia/Seoul")).plusDays(4).withNano(0).toString(),
                "type", ReservationType.EARLY_BIRD.name(),
                "ticketURL", "https://ticket.example.com/updated",
                "remark", "업데이트"
        );
        mockMvc.perform(put("/api/admin/performance/{performanceId}/reservation/{reservationInfoId}", performanceId, 1L)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        // delete seeded reservation id 1
        LocalDateTime deleteStart = LocalDateTime.now(ZoneId.of("Asia/Seoul"));
        mockMvc.perform(delete("/api/admin/performance/{performanceId}/reservation/{reservationInfoId}", performanceId, 1L))
                .andExpect(status().isOk());
